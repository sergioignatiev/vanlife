<template>
  <div v-if="item" class="flex gap-[15px] flex-col">
    ...
    <div class="capitalize">
      <span class="font-extrabold">Category:</span> {{ item.type }}
    </div>
    <div class="leading-[22px]">
      <span class="font-extrabold">Description:</span> {{ item.description }}
    </div>
    <div><span class="font-extrabold">Visibility:</span> Public</div>
  </div>

  <div v-else class="text-red-600 font-bold">🚨 Такого id нет: {{ id }}</div>
</template>
